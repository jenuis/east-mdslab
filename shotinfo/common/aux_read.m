...
if ~haselement(exception, 'lhw') && haselement({'raw', 'all','lhw'}, heat_type)
    lhw = signalread(shotno, 'east_1',    {'\plhi', '\plhr'});
    if ~haselement(exception, 'plhi') && fieldexist(lhw, 'plhi') && lhw.plhi.status
        plhi = signaldownsample(lhw.plhi);
        plhr = signaldownsample(lhw.plhr);
...
icrf = signalread(shotno, 'power_east', node_list);
for i=1:length(node_list)
    ...
    if ~haselement(exception, node_name) && fieldexist(icrf, node_name) && icrf.(node_name).status
        tmp = signaldownsample(icrf.(node_name));
...
%% ecrh
if ~haselement(exception, 'ech') && haselement({'raw', 'all','ech'}, heat_type)
    ech = signalread(shotno, 'ecrh_east', '\pecrh1i');
    if ~haselement(exception, 'pecrh1i') && fieldexist(ech, 'pecrh1i') && ech.pecrh1i.status
        pecrh1i = signaldownsample(ech.pecrh1i);
...
%% icrf
if ~haselement(exception, 'icrf') && haselement({'raw', 'all','icrf'}, heat_type)
    icrf = signalread(shotno, 'icrf_east', {'\picrfii', '\picrfbi'});
    if ~haselement(exception, 'picrfii') && fieldexist(icrf, 'picrfii') && icrf.picrfii.status 
        picrfii = signaldownsample(icrf.picrfii);
    ...
    if ~haselement(exception, 'picrfbi') && fieldexist(icrf, 'picrfbi') && icrf.picrfbi.status
        picrfbi = signaldownsample(icrf.picrfbi);
...
%% lhw
if ~haselement(exception, 'lhw') && haselement({'raw', 'all','lhw'}, heat_type)
    lhw = signalread(shotno, 'east_1',    {'\plhi1', '\plhi2', '\plhr1', '\plhr2'});
    if ~haselement(exception, 'plhi1') && fieldexist(lhw, 'plhi1') && lhw.plhi1.status
        plhi1 = signaldownsample(lhw.plhi1);
        plhr1 = signaldownsample(lhw.plhr1);
    ...
    if ~haselement(exception, 'plhi2') && fieldexist(lhw, 'plhi2') && lhw.plhi2.status
        plhi2 = signaldownsample(lhw.plhi2);
        plhr2 = signaldownsample(lhw.plhr2);
...
%% nbi
if ~haselement(exception, 'nbi') && haselement({'raw', 'all','nbi'}, heat_type)
    nbi1  = signalread(shotno, 'nbi_east',  {'pnbi1rsource', 'pnbi1lsource'});
    nbi2  = signalread(shotno, 'nbi_east',  {'pnbi2rsource', 'pnbi2lsource'});
    ...
    nbi1 = signaldownsample( signalmerge(nbi1)); 
    nbi2 = signaldownsample( signalmerge(nbi2));
    if nbi1.status
...
pow_signal = signalslice(pow_signal, [0 pow_signal.time(end)]);
